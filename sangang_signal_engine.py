...
__version__ = "2.9.7-2026-07-19-live-price-patch"
...
class SangangEngine:
    ...
    def evaluate(
        ...
        if matched_level is not None:
            # 터치 횟수는 당일(오늘 세션) 30분봉 데이터만으로 계산
            last_date30 = df30.index[-1].date()
            session_df30 = df30[df30.index.date == last_date30]
            touch_count = count_prior_touches(
                session_df30,
                matched_level,
                tolerance_pct=self.level_tolerance_pct,
                lookback_bars=len(session_df30),
            )
            reliability_label = touch_confidence_label(touch_count)

        extreme_emphasis = None
        if channel_info is not None:
            extreme_emphasis = check_channel_extreme_emphasis(
                df3, channel_info, direction, touch_count=touch_count
            )
